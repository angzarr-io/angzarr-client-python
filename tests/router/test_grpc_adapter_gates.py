...
class _StubContext:
    """Minimal grpc.ServicerContext stand-in capturing abort() calls."""
...
def test_handle_fact_returns_unimplemented_when_no_fact_handlers():
    router = Mock()
    ...
    with pytest.raises(_Aborted):
        adapter.HandleFact(FactRequest(), ctx)
    ...
    # Critical: dispatch_fact NEVER called when gate fails.
    router.dispatch_fact.assert_not_called()


def test_handle_fact_dispatches_when_supported():
    router = Mock()
    ...
    request = FactRequest()
    result = adapter.HandleFact(request, ctx)
...
def test_replay_returns_unimplemented_when_not_opted_in():
    router = Mock()
    ...
    with pytest.raises(_Aborted):
        adapter.Replay(ReplayRequest(), ctx)
...
def test_replay_dispatches_when_opted_in():
    router = Mock()
    ...
    result = adapter.Replay(request, ctx)
...
def test_handle_does_not_consult_fact_or_replay_gates():
    ...
    adapter.Handle(ContextualCommand(), ctx)
