...
def _given_payment_handler(world, domain="payment"):
    world.classes["__payment_handlers__"] = []


def _register_payment_compensator(world, src_domain: str, cmd: str) -> None:
...
def _given_rejected(world, src_domain, cmd):
    _register_payment_compensator(world, src_domain, cmd)
...
def _given_payment_compensates_releasing(world):
    _register_payment_compensator(world, "inventory", "ReserveStock")
